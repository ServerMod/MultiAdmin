...
public class CustomNetworkManager : NetworkManager
{
    ...
    public void UpdateMotd()
    {
        ...
        string text2 = ConfigFile.GetString("server_name", "[nick]'s game");
        new IPAddress(BitConverter.GetBytes(IPAddress.HostToNetworkOrder((int)SteamGameServer.GetPublicIP()))).ToString();
        ...
        text2 = text2.Replace("$ip", base.networkPort.ToString());
    ...
    private List<Lobby> steam_lobbies;
    ...
    public void CloseOrphanedLobbies(int current_port)
    {
        ...
        if (File.Exists(Application.persistentDataPath + "/lobby.registry"))
        {
            ...
            steam_lobbies = (List<Lobby>)bf.Deserialize(file);
            file.Close();
        }

        if (steam_lobbies != null)
        {
            foreach (Lobby lobby in steam_lobbies)
            {
    ...
    public void SaveCurrentLobby(CSteamID current_id, int current_port)
    {
        if (steam_lobbies == null)
        {
            steam_lobbies = new List<Lobby>();
        }
        steam_lobbies.Add(new Lobby(current_id, current_port));
        BinaryFormatter bf = new BinaryFormatter();
        FileStream file = File.Create(Application.persistentDataPath + "/lobby.registry");
        bf.Serialize(file, steam_lobbies);
        file.Close();
    }
