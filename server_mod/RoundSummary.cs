...
public class RoundSummary : NetworkBehaviour
{
	...
	protected static void InvokeCmdCmdSetSummary(NetworkBehaviour roundSum, NetworkReader reader)
	{
		if (!NetworkServer.active)
		{
			Debug.LogError("Command CmdSetSummary called on client.");
		...
		((RoundSummary) roundSum).CmdSetSummary(GeneratedNetworkCode._ReadSummary_RoundSummary(reader), (int) reader.ReadPackedUInt32());
	}
